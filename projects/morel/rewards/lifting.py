...
offset = np.array([0.665, 0.08, 0.682])
...
def reward_function(paths):
    ...
    for i in range(num_traj):
        ...
        transformed_tags_xyz = [coor_transform(tag_) for tag_ in tags_xyz]
        ...
        distance = np.linalg.norm(transformed_tags_xyz - ee_xyz, axis=1)
        assert distance.shape[0] == horizon

        height = np.array(transformed_tags_xyz)[:, 2] # move from 0.49 to 0.62

        rewards[i] = 0.8 * (0.5 - distance) + 2.5 * (height - 0.49)
